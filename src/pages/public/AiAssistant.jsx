                        ...
                        <div className="p-10 border-t border-clinical bg-white/80 backdrop-blur-md relative z-30">
                            <div className="max-w-[1200px] mx-auto relative group">
                                ...
                                <div className="absolute right-6 top-1/2 -translate-y-1/2 flex items-center gap-4">
                                    ...
                                    {showAttachmentMenu && (
                                        <div className="absolute bottom-20 right-0 bg-white/95 backdrop-blur-xl border border-clinical shadow-[0_40px_80px_rgba(0,0,0,0.2)] rounded-[1.25rem] p-4 w-[420px] z-50 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                                            <div className="flex items-center justify-between mb-4 px-2">
                                                <p className="text-[9px] font-black text-primary/30 uppercase tracking-[0.4em]">ATTACHMENT PROTOCOLS</p>
                                                <div className="flex gap-1.5 text-clinical">
                                                    <Icon name="verified_user" size={14} className="opacity-20" />
                                                </div>
                                            </div>
                                            
                                            <div className="grid grid-cols-2 gap-3">
                                                {[
                                                    { label: 'Local Node', desc: 'Secure upload', icon: 'file_upload', action: triggerFileSelect },
                                                    { label: 'Document Vault', desc: 'Encrypted vault', icon: 'lock', action: () => handleSimulatedAttach('Vault_Registry_Archive.pdf') },
                                                    { label: 'Entity Ledger', desc: 'Firm records', icon: 'business_center', action: () => handleSimulatedAttach('Entity_Profile_Dossier.json') },
                                                    { label: 'Statute Map', desc: 'Legal codes', icon: 'gavel', action: () => handleSimulatedAttach('Statutory_Provision_Ref.pdf') }
                                                ].map((opt, i) => (
                                                    <button 
                                                        key={i}
                                                        onClick={opt.action}
                                                        className="flex flex-col gap-3 p-4 bg-white/[0.03] border border-clinical rounded-xl transition-all group text-left hover:bg-white hover:shadow-clinical hover:border-secondary/20 hover:-translate-y-1"
                                                    >
                                                        <div className="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-lg text-primary/30 group-hover:text-secondary group-hover:bg-secondary/5 transition-all">
                                                            <Icon name={opt.icon} size={20} />
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-black text-primary uppercase tracking-widest leading-none mb-1.5">{opt.label}</p>
                                                            <p className="text-[8px] text-primary/30 font-bold uppercase tracking-[0.1em] leading-tight">{opt.desc}</p>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>

                                            <div className="mt-4 pt-4 border-t border-clinical text-center">
                                                <p className="text-[8px] font-bold text-primary/10 uppercase tracking-[0.3em]">SELECT SOURCE NODE</p>
                                            </div>
                                        </div>
                                    )}
